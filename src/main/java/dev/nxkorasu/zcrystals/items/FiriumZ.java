...
import static dev.nxkorasu.zcrystals.util.ZCrystals.FIRIUM_Z;
...
public class FiriumZ extends SimplePolymerItem {
    ...
    public int getPolymerCustomModelData(ItemStack itemStack, @Nullable ServerPlayerEntity player){
        this.modelData = ZCrystals.firiumZModelData;
    ...
    private void RemoveItem(ItemUsageContext itemUsageContext){
        ...
        if(!offHand.getItem().equals(FIRIUM_Z)){
    ...
    public ActionResult useOnBlock(ItemUsageContext itemUsageContext) {
        ...
        ItemStack offHand = Objects.requireNonNull(itemUsageContext.getPlayer()).getOffHandStack();
        if(!offHand.getItem().equals(FIRIUM_Z) || !heldStack.getItem().equals(FIRIUM_Z)){
            if(block == Blocks.SCULK){
                RemoveItem(itemUsageContext);
                player.giveItemStack(new ItemStack(ZCrystals.INCINIUM_Z));
